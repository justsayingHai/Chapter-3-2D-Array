...
int main()
{
    ...
    for (int row = 0; row < cashiers; row++)
    {
        ...
        if (total > overall_max_sales)
        {
            overall_max_sales = total;
            best_cashier_index = row + 1;
        }
    }

    cout << "The best cashier is Cashier " << best_cashier_index << "(" << overall_max_sales << "$)" << endl;
